...
- Первая строка описывает назначение компонента.
- `ComponentName` в шаблоне замени именем документируемого компонента.
- После одной пустой строки добавляй блок `Используется для:`.
- Указывай от одного до трёх реальных сценариев применения.
- Не пересказывай props, JSX-структуру и детали реализации.

```tsx
/**
 * <Назначение ComponentName в 1 строке>.
 *
 * Используется для:
 *  - <сценарий 1>
 *  - <сценарий 2, если есть>
 *  - <сценарий 3, если есть>
 */
```
...
- React-компонент имеет верхнеуровневый JSDoc по обязательному шаблону с 1-3 сценариями применения.
